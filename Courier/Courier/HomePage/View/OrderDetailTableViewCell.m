...
- (void)gotoPick {
    ...
    OrderDetailTableViewCell *cell = self;
            ...
            if ([latitude isEqualToString:@"20"] || [longitude isEqualToString:@"20"]) {// 已开启定位功能
                ...
                [session POST:REQUESTURL parameters:@{@"key":parameter} progress:^(NSProgress * _Nonnull uploadProgress) {
                    ...
                    if (!result.integerValue) {
                        NSLog(@"前往取货地点成功");
                        [cell.delegate orderRefreshWithMessage:cell.btnStatus status:@"成功"];
                    } else {
                        NSLog(@"前往取货地点失败");
                        [cell.delegate orderRefreshWithMessage:cell.btnStatus status:@"失败"];
                    }
